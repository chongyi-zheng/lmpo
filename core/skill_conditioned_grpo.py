...
def main(_):
    ...
    proposer, proposer_params = create_model_from_ckpt(ckpt_dir)
    model, model_params = create_model_from_ckpt(ckpt_dir)
    tx = optax.chain(
    ...
    def update(train_state: TrainState, token_batch, mask, advantages, old_logprobs):
        ...
        def loss_fn(grad_params):
            ...
            # (chongyi): double check this average entropy implementation
    ...
    for i in tqdm.tqdm(range(FLAGS.num_steps)):
        ...
        while len(buffer_tokens) < FLAGS.groups_per_batch:
            ...
            # proposer_action_tokens = autoregressive_sample(
            #     train_state.model_def, train_state.params, proposer_tokens, rng=key, num_generation_tokens=num_generation_tokens,
            ...
            proposer_action_tokens, _ = autoregressive_sample(
                train_state.model_def, train_state.params, proposer_prompt_tokens,
            ...
            proposer_all_tokens = jnp.concatenate([proposer_prompt_tokens, proposer_action_tokens], axis=-1)
            ...
            new_states, _, returns_local, dones, env_infos = env.step_list(env_states,
                                                                           [t.tolist() for t in action_tokens_local])
            ...
            for group_idx in range(advantages_grouped.shape[0]):
                if np.all(advantages_grouped[group_idx, :] == 0) and FLAGS.do_group_filter:
                    ...
                    buffer_tokens.append(proposer_all_tokens_grouped[group_idx, :])
